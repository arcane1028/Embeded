...
void BSP_LED_Init(Led_TypeDef Led)
{
	...
	GPIO_InitStruct.Pin = GPIO_PIN[Led];															// GPIO_PIN 번호 Init
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;					// GPIO MODE를 OUPUT_PP로 설정. alternative 설정시 GPIO로 사용하지 못하고 Hardware적으로 정해진 임무만 수행하게된다.
	GPIO_InitStruct.Pull = GPIO_PULLUP;																// 플로팅상태(알수없는 상태)의 전압을 High 상태로 맞춰 준다. 반대로 Pull Down은 Low 상태로 맞춤.
	GPIO_InitStruct.Speed = GPIO_SPEED_FAST;										// GPIO SPEED 설정 - 2MHz, 25MHz, 50MHz, 100MHz 로 다양하다.
