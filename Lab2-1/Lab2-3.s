...
; 학번 : 201302480 이름 : 정석현
				...
				LDR 	R0, = 5					; R1 레지스터에 5 저장
				LDR	R1, =10					; R2 레지스터에 10 저장
				LDR	R2, =7					; R3 레지스터에 7 저장

				MUL R1, R1, R2 			; R1 레지스터에 R1과 R2 곱한 값을 저장
				
				CMP	R0, #5					; R0에 5 뺀 값을 flag에 반영
				SUB R3, R0, R2			; 임시로 R3 레지스터에 R0과 R2 레지스터 뺀 값을 저장
				ADDNE	R1, R1, R3		; flag가 0이 아니면 R1과 R3 더한 값을 R1에 저장
